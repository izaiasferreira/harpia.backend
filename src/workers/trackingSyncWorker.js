...
async function getAgentProfileCached(agentId) {
    ...
    // Busca do banco
    const { rows } = await sinergia_pool.query(`SELECT estado, "Cargo" as cargo, regional, seccional FROM colaboradores WHERE lower("ID") = $1 LIMIT 1`, [agentId.toLowerCase()]);
...
async function processBatch(rows) {
    ...
    if (uniqueAgentIds.length > 0) {
        ...
        const { rows: validAgentsRows } = await sinergia_pool.query(
