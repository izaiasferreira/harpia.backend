...
async function checkToken(req, res) {
    ...
    if (!rawToken.startsWith('sinergia_') && !rawToken.startsWith('gedai_') && !rawToken.startsWith('cenos_')) {
        res.status(401).json({ error: 'Token inválido (deve começar com sinergia_)' });
        return false;
    }
